...
+ (void) DISABLED_testDecodeH264ToMvid_WithCompositeReader
{  
  ...
  AVURLAsset *avUrlAsset = [[[AVURLAsset alloc] initWithURL:fileURL options:options] autorelease];
  ...
  if (TRUE) {
    ...
    // QTRLE = kCMVideoCodecType_Animation
    ...
    // Emit 32BGRA (Alpha would always be 0xFF)
        
    NSDictionary *videoOptions = [NSDictionary dictionaryWithObject:
  ...
  while ([aVAssetReader status] == AVAssetReaderStatusReading)
  {
    ...
    buffer = [aVAssetReaderOutput copyNextSampleBuffer];
    
    NSLog(@"WRITTING...");
   
    if (buffer) {
      UIImage *image = imageFromSampleBuffer(buffer);
...
+ (void) testDecodeH264ToMvid_WithTrackReader
{
  NSString *resourceName = @"32x32_black_blue_h264.mov";
  NSString *resPath = [AVFileUtil getResourcePath:resourceName];
  ...
  // FIXME: pick right one to match default iOS native format (I think ABGR)
  // kCVPixelFormatType_24BGR
  // kCVPixelFormatType_32BGRA
  // kCVPixelFormatType_32ABGR
  
  NSDictionary *videoSettings = nil; // can't be nil!
  
  if (FALSE) {
    // Does not work!
    videoSettings = [NSDictionary dictionaryWithObject:
                     [NSNumber numberWithInt:kCVPixelFormatType_32ABGR] forKey:(id)kCVPixelBufferPixelFormatTypeKey];    
  } else if (FALSE) {
    // Nope
    videoSettings = [NSDictionary dictionaryWithObject:
                     [NSNumber numberWithUnsignedInt:kCVPixelFormatType_24BGR] forKey:(id)kCVPixelBufferPixelFormatTypeKey];
  } else if (TRUE) {
    // YES
    videoSettings = [NSDictionary dictionaryWithObject:
                     [NSNumber numberWithUnsignedInt:kCVPixelFormatType_32BGRA] forKey:(id)kCVPixelBufferPixelFormatTypeKey];
